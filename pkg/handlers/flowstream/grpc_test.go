...
func TestFilterToGetFlowsRequest(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, tt.wantDirection, req.Filters[0].Direction)
			assert.Equal(t, tt.wantNamespaces, req.Filters[0].Namespaces)
			assert.Equal(t, tt.wantPodNames, req.Filters[0].PodNames)
			assert.Equal(t, tt.wantServices, req.Filters[0].ServiceNames)
