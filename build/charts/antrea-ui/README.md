...
| plugins.maxDirectoryPlugins | int | `10` | Same as maxConfigMapPlugins, but for the backend's optional filesystem-directory plugin source (see docs/plugins.md) - not itself exposed as a chart value, since how a deployment would deliver a shared plugin directory to the backend Pod isn't settled yet. |
