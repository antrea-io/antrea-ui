...
| flowAggregator.maxConcurrentSubscriptions | int | the backend's own built-in default | Caps how many flow streams the backend keeps open against the Flow Aggregator at once. The Flow Aggregator's own maxStreamsPerClientIP keys on the backend's source IP, so every antrea-ui user shares that one budget; this bounds antrea-ui's own usage of it so hitting the cap is an immediate, clear error rather than a shared limit discovered as a gRPC ResourceExhausted error. Leave unset to use the backend's own built-in default. |
