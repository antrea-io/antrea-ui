...
| plugins.maxConfigMapBundleBytes | int | `5242880` | Caps how many bytes a single ConfigMap-backed plugin's bundle.zip may decompress to in total. A ConfigMap's own ~1MiB etcd size limit only bounds the compressed bytes, not what they decompress to, so this is what actually protects the backend from a maliciously high-ratio archive ("zip bomb") exhausting its memory. 0 means unbounded. |
