...
| flowAggregator.insecureSkipVerify | bool | `false` | Disable TLS server certificate verification. Should only be used for development or testing; never enable this in production. With this on, any server answering at `address` is trusted: the backend hands it every flow viewer's credential (a bearer token, or a client certificate on the connection) and, for admin-password sessions, a freshly minted antrea-ui-admin token - all replayable against the real Kubernetes API server by whoever controls that address. |
