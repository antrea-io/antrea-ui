...
| flowAggregator.caConfigMap | string | `"flow-aggregator-ca"` | Name of the ConfigMap (in namespace below) containing the CA certificate (key: ca.crt) used to verify the FlowStreamService server certificate. Leaving this empty does not skip server certificate verification: it falls back to the system trust store, which will not verify the Flow Aggregator's self-signed certificate, and the connection will fail. Use insecureSkipVerify below for that (dev/test only). |
| flowAggregator.enabled | bool | `false` | When true, the backend connects to Flow Aggregator's FlowStreamService over gRPC. |
| flowAggregator.insecureSkipVerify | bool | `false` | Disable TLS server certificate verification. Should only be used for development or testing; never enable this in production. With this on, any server answering at `address` is trusted: the backend hands it every flow viewer's bearer token and, for admin-password sessions, a freshly minted antrea-ui-admin token - all replayable against the real Kubernetes API server by whoever controls that address. A client certificate cannot be replayed, since its private key never leaves antrea-ui, but the server still learns the viewer's identity and can serve fake flow data. |
...
| plugins.directory | string | `""` | Filesystem path (inside the backend container) the backend also watches for plugin bundles - one subdirectory per plugin, each holding a manifest.json plus a bundle.zip (see docs/plugins.md). Meant as a fallback for a plugin whose bundle.zip outgrows a ConfigMap's ~1MiB cap: mount whatever shared volume actually delivers the bundle there (a hostPath, a PVC, ...) via the top-level extraVolumes/backend.extraVolumeMounts values, then point this at that mount path. Empty (the default) disables directory-based loading entirely; ConfigMap-based loading is unaffected either way. |
...
| plugins.maxBundleBytes | int | `10485760` | Caps how many bytes a single plugin's bundle.zip may decompress to in total, shared by both sources. A ConfigMap's own ~1MiB etcd size limit only bounds the compressed bytes, not what they decompress to, and a plugin directory has no equivalent limit at all, so this is what actually protects the backend from a maliciously high-ratio archive ("zip bomb") exhausting its disk. 0 means unbounded. |
| plugins.maxConfigMapPlugins | int | `10` | Caps how many distinct ConfigMap-backed plugins the backend tracks at once. A new plugin past this limit is rejected and logged; updates to an already-tracked one are never blocked by it. 0 means unbounded. |
| plugins.maxDirectoryPlugins | int | `10` | Same as maxConfigMapPlugins, but for the directory source above. 0 means unbounded. |
| plugins.namespace | string | `""` | Namespace to watch for plugin ConfigMaps. Empty (the default) disables the ConfigMap plugin source entirely, and with it the Role/RoleBinding that would otherwise grant antrea-ui's ServiceAccount get/list/watch on every ConfigMap in that namespace. Set this to enable the source - antrea-ui is commonly installed into kube-system, which can host other sensitive ConfigMaps, so docs/plugins.md recommends a dedicated namespace over reusing the release namespace. If set to anything other than the release namespace, whoever runs `helm install`/`upgrade` needs permission to create a Role/RoleBinding in that other namespace too. |
| plugins.signature.enabled | bool | `false` | When true, every plugin must ship a signature that verifies against at least one of trustedKeys below, and a manifest bundleSha256 matching its bundle.zip. Plugins that fail either check are not loaded (an error is logged). Requires at least one trustedKeys entry. Only protects against whoever writes plugin ConfigMaps if plugins.namespace is a dedicated namespace, not the release namespace, which holds the backend configuration and the trusted keys. See docs/plugins.md. |
| plugins.signature.trustedKeys | list | `[]` | Keys a plugin may be signed by: a plugin loads when its signature verifies against any one of them. Each entry has a `name` (a DNS-1123 label, unique within the list, used in the backend's logs and in the key file's mount path), a `type` (only `openpgp` today: an OpenPGP public key file, which may hold several keys, and plugins signed with a detached manifest.json.asc), and a `configMap` (`name` and `key`) selecting the ConfigMap in the release namespace, and the key within it, that holds the key file. You create these ConfigMaps yourself (see docs/plugins.md); the chart deliberately does not, since a trust anchor does not belong in values.yaml. Ignored unless enabled is true: with enabled false, plugins load unverified even when keys are listed here. |
