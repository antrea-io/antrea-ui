...
export class FlowStreamClient {
    ...
    updateFilter(filter: FlowStreamFilter): void {
        ...
        if (this.running) {
            this.abortController?.abort();
            // Flush any buffered flows from the old filter before reconnecting so
            // they are not delivered under the new filter context.
            this.flushBatch();
            this.batchBuffer = [];
            this.reconnectAttempts = 0;
            this.connect();
    ...
    private parseSSEBuffer(buffer: string): { parsed: SSEEvent[]; remaining: string } {
        ...
        // Normalize CRLF to LF to handle proxies that rewrite line endings.
        ...
        for (const block of blocks) {
            ...
            for (const line of block.split('\n')) {
                if (line.startsWith('event:')) {
                    ...
                    // Strip only the single leading space after 'data:' (if present).
